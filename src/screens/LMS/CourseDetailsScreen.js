...
const CourseDetailsScreen = ({ navigation, route }) => {
  ...
  const course = {
    ...
    description: 'Learn the fundamentals of React Native development from scratch. This comprehensive course covers everything you need to know to build mobile applications using React Native.',
    instructor: 'Jane Smith',
    thumbnail: 'https://via.placeholder.com/400x250/4A90E2/FFFFFF?text=React+Native+Course',
      ...
      <View style={styles.courseInfo}>
        ...
        <View style={styles.statsGrid}>
          <Card style={styles.statCard}>
            <Text style={styles.statNumber}>{course.lessons.filter(l => l.completed).length}</Text>
            <Text style={styles.statLabel}>Lessons Completed</Text>
          </Card>
          <Card style={styles.statCard}>
            <Text style={styles.statNumber}>{course.quizzes.filter(q => q.completed).length}</Text>
    ...
    <FlatList
      ...
      renderItem={({ item, index }) => (
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('LessonPlayer', { lessonId: item.id, courseId: course.id })}
        >
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        <TouchableOpacity 
          ...
          onPress={() => navigation.navigate('QuizScreen', { quizId: item.id, courseId: course.id })}
        >
          ...
          <View style={styles.quizMeta}>
            ...
            {item.completed && (
              <View style={styles.quizMetaItem}>
                ...
                <Text style={[styles.quizMetaText, { color: COLORS.success }]}>{item.score}%</Text>
              </View>
            )}
          </View>
        </TouchableOpacity>
